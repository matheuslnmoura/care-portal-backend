...
import { ConflictError, DuplicateEmailError, DuplicatePhoneError, NotFoundError, TenantNotFoundError } from '../../../../../exceptions/exceptions.js';
...
describe('StaffUserService', () => {
  ...
  describe('createUser', () => {
    ...
    it('throws NotFoundError when the repository throws TenantNotFoundError', async () => {
      mockRepository.create.mockRejectedValueOnce(new TenantNotFoundError());

      await expect(staffUserService.createUser({ user: createPayloadFixture() })).rejects.toThrow(NotFoundError);
    });

    it('includes the TENANT_NOT_FOUND code when TenantNotFoundError is thrown', async () => {
      mockRepository.create.mockRejectedValueOnce(new TenantNotFoundError());
